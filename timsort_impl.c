...
static int kisbigger(void *x,
                     const struct list_head *node1,
                     const struct list_head *node2)
{
...
void q_timsort(struct list_head *head, bool descend)
{
    ...
    list_cmp_func_t cmp_func;
    if (descend)
        cmp_func = &kissmaller;
    else
        cmp_func = &kisbigger;
    timsort(NULL, head, cmp_func);
}